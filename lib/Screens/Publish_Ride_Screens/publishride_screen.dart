...
class _PublishRideScreenState extends State<PublishRideScreen> {
  ...
  void decrementPrice() {
    final current = int.tryParse(priceController.text) ?? 500;
    if (current > 50) setState(() => priceController.text = (current - 50).toString());
  }
  ...
  Future<void> selectDate() async {
    final DateTime? picked = await showDatePicker(
      ...
      builder: (context, child) => Theme(
        data: Theme.of(context).copyWith(colorScheme: const ColorScheme.light(primary: Color(0xFFFF4444))),
        child: child!,
      ),
    );
    if (picked != null) setState(() => dateController.text = '${picked.day}/${picked.month}/${picked.year}');
  }
  ...
  void handleSubmit() {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (context) => PublishsucessScreen()));
  }
  ...
  Future<void> saveRideToFirebase() async {
    try {
      ...
      if (profileImage != null) {
        final storageRef = FirebaseStorage.instance.ref().child("ride_profiles/${DateTime.now().millisecondsSinceEpoch}.jpg");
      ...
      await FirebaseFirestore.instance.collection("rides").add({
        ...
        "fromLatLng": fromLatLng != null ? {"lat": fromLatLng!.lat, "lng": fromLatLng!.lng} : null,
        "toCity": toCityController.text.trim(),
        "toLatLng": toLatLng != null ? {"lat": toLatLng!.lat, "lng": toLatLng!.lng} : null,
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Ride Published Successfully!")));
  ...
  Future<void> fetchPlaceSuggestions(String input, {required bool isFrom}) async {
    ...
    try {
      setState(() {
        if (isFrom) _isLoadingFrom = true; else _isLoadingTo = true;
      });
      ...
      if (response.statusCode == 200) {
        ...
        if (status == 'OK') {
          ...
          setState(() {
            if (isFrom) { fromSuggestions = []; showFromSuggestions = false; } else { toSuggestions = []; showToSuggestions = false; }
          });
        } else {
          debugPrint('Places Autocomplete API returned status: $status');
          if (status == 'REQUEST_DENIED') {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Places API request denied. Check API key & billing.')));
          }
          setState(() {
            if (isFrom) { fromSuggestions = []; showFromSuggestions = false; } else { toSuggestions = []; showToSuggestions = false; }
          });
      ...
      setState(() {
        if (isFrom) _isLoadingFrom = false; else _isLoadingTo = false;
      });
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (status == 'OK') {
          ...
          if (status == 'REQUEST_DENIED') {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Place Details request denied. Check API key & billing.')));
          }
          setState(() {
            if (isFrom) { fromSuggestions = []; showFromSuggestions = false; } else { toSuggestions = []; showToSuggestions = false; }
          });
  ...
  Widget _buildTextField(
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        errorStyle: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w500),
        filled: true,
                  ...
                  child: Padding(
                    ...
                    child: Form(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Center(
                          child: Text("Publish A Ride?", style: GoogleFonts.lexend(fontSize: screenWidth * 0.065, fontWeight: FontWeight.w600, color: Colors.white)),
                        ),
                        SizedBox(height: screenHeight * 0.025),

                        // Rider Details
                        Text("Enter Rider Details", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),

                        Text("Name", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.010),
                        _buildTextField("Enter Rider Name", riderNameController, screenWidth, screenHeight,
                            keyboardType: TextInputType.name, validator: (v) {
                        ...
                        // Phone
                        Text("Phone Number", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.010),
                        _buildTextField("Enter Phone Number", phoneController, screenWidth, screenHeight,
                        ...
                        // Enter Ride Details
                        Text("Enter Ride Details", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),

                        // From - autocomplete field
                        Text("From", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.010),
                        _buildTextField("Enter City / Place", fromCityController, screenWidth, screenHeight,
                        ...
                        Text("To", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.010),
                        _buildTextField("Enter City / Place", toCityController, screenWidth, screenHeight,
                            validator: (v) {
                              if (v == null || v.isEmpty) return "Select a City";
                              if (v == fromCityController.text) return "From & To cannot be same";
                              return null;
                            },
                        ...
                        // Passengers
                        Text("No of Passengers", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),
                        Row(children: [
                          GestureDetector(onTap: decrementPassengers, child: Container(width: screenWidth * 0.1, height: screenWidth * 0.1, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: const Icon(Icons.remove, color: Color(0xFFFF4444)))),
                          SizedBox(width: screenWidth * 0.04),
                          Text("$passengers", style: GoogleFonts.lexend(fontSize: screenWidth * 0.05, fontWeight: FontWeight.w600, color: Colors.white)),
                          SizedBox(width: screenWidth * 0.04),
                          GestureDetector(onTap: incrementPassengers, child: Container(width: screenWidth * 0.1, height: screenWidth * 0.1, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: const Icon(Icons.add, color: Color(0xFFFF4444)))),
                        ]),
                        SizedBox(height: screenHeight * 0.02),

                        // Date & Times
                        Text("Enter Ride Timings", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),
                        Text("Date", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),
                        _buildTextField("Enter Date", dateController, screenWidth, screenHeight, readOnly: true, onTap: selectDate, suffixIcon: Icons.calendar_today, validator: (v) => v == null || v.isEmpty ? "Please select a date" : null),
                        SizedBox(height: screenHeight * 0.015),
                        Text("Pickup Time", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),
                        _buildTextField("Enter Pickup Time", pickupTimeController, screenWidth, screenHeight, readOnly: true, onTap: () => selectTime(pickupTimeController), suffixIcon: Icons.access_time, validator: (v) => v == null || v.isEmpty ? "Pickup time required" : null),
                        SizedBox(height: screenHeight * 0.015),
                        Text("Drop Time", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),
                        _buildTextField("Enter Drop Time", dropTimeController, screenWidth, screenHeight, readOnly: true, onTap: () => selectTime(dropTimeController), suffixIcon: Icons.access_time, validator: (v) => v == null || v.isEmpty ? "Drop time required" : null),
                        SizedBox(height: screenHeight * 0.02),

                        // Price
                        Text("Enter Ride Price", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),
                        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                          GestureDetector(onTap: decrementPrice, child: Container(width: screenWidth * 0.1, height: screenWidth * 0.1, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: const Icon(Icons.remove, color: Color(0xFFFF4444)))),
                          SizedBox(width: screenWidth * 0.04),
                          Container(
                            padding: EdgeInsets.symmetric(horizontal: screenWidth * 0.06, vertical: screenHeight * 0.01),
                            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(25)),
                            child: Row(children: [
                              const Icon(Icons.currency_rupee, color: Color(0xFFFF4444)),
                              SizedBox(width: screenWidth * 0.15, child: TextFormField(controller: priceController, textAlign: TextAlign.center, keyboardType: TextInputType.number, validator: (v) {
                                if (v == null || v.isEmpty) return "Enter price";
                                if (!RegExp(r'^[0-9]+$').hasMatch(v)) return "Only numbers allowed";
                                return null;
                              }, decoration: const InputDecoration(border: InputBorder.none, isDense: true, errorStyle: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w500)))),
                            ]),
                          ),
                          SizedBox(width: screenWidth * 0.04),
                          GestureDetector(onTap: incrementPrice, child: Container(width: screenWidth * 0.1, height: screenWidth * 0.1, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: const Icon(Icons.add, color: Color(0xFFFF4444)))),
                        ]),
                        SizedBox(height: screenHeight * 0.02),

                        // Description
                        Text("Enter Description ", style: GoogleFonts.lexend(fontSize: screenWidth * 0.04, fontWeight: FontWeight.w500, color: Colors.white)),
                        SizedBox(height: screenHeight * 0.012),
                        TextFormField(controller: descriptionController, maxLines: 3, decoration: InputDecoration(hintText: "Enter description...(Pickup Place - Drop Place)", hintStyle: GoogleFonts.lexend(color: Colors.grey[400], fontSize: screenWidth * 0.038), filled: true, fillColor: Colors.white, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), contentPadding: EdgeInsets.symmetric(horizontal: screenWidth * 0.04, vertical: screenHeight * 0.018))),
                        SizedBox(height: screenHeight * 0.025),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            child: Text("Submit", style: GoogleFonts.lexend(fontSize: screenWidth * 0.045, fontWeight: FontWeight.w600, color: const Color(0xFFFF4444)))),
                        ),
                        SizedBox(height: screenHeight * 0.02),
                      ]),
                    ),
      ...
      body: SafeArea(
        child: Stack(children: [
          ...
          Positioned(
            ...
            right: 20,
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12)), onPressed: () => Navigator.pop(context, false), child: const Text("Retake")),
              ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.green, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12)), onPressed: () => Navigator.pop(context, true), child: const Text("Save")),
            ]),
          ),
        ]),
      ),
